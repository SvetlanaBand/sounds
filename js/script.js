...
let ul = document.querySelectorAll(".list__choise");
let list = document.querySelectorAll(".list__choise-item");
let lists = document.querySelector(".list__choice-items");
let audioName = document.querySelector("audio");
let p = document.getElementById("p");
let img = document.getElementById("img");
let showImg = document.querySelector(".shou__play-img");
let showName = document.querySelector(".shou__play-name");
let showPlay = document.querySelector(".shou__play-sounds");
let showAudio = document.querySelector(".shou__play-audio");
let showText = document.querySelector(".shou__text");
let active = document.querySelector(".list__choice-items .active");
...
function showRandom() {
  ...
  while (cards.length < 6) {
    ind = Math.floor(Math.random() * birdsData.length);
    console.log(birdsData.length)
    if (cards.indexOf(ind) === -1) {
      cards.push(ind);
    }
  }
  console.log(cards);
...
function showChoice() {
  showRandom();
  audioName.src = birdsData[1][cards[ind]].audio;
  console.log(birdsData[1][cards[ind]])
  for (let i = 0; i < 6; i++) {
    list[i].innerHTML = birdsData[1][cards[i]].name;

    console.log(list[i].innerHTML);
  }
}
showChoice();
console.log(birdsData[1][cards[ind]].name)
function checkAnswer(event) {
  if (event.target.tagName != "LI") return;
  
  //if (active) active.classList.remove("active");
 // event.target.classList.add("active");
  event.target.innerHTML == birdsData[1][cards[ind]].name
    ? showDiscription(event)
    : getBox(event);
}
...
function showDiscription(e) {
  img.src = birdsData[1][cards[ind]].image;
  p.innerHTML = birdsData[1][cards[ind]].name;
  console.log(birdsData[1][cards[1]].name);
  console.log(p.innerHTML);
  
  e.target.classList.add("active");
  let audio = new Audio;
    audio.src = '../audio/55031879babb424.mp3'
    audio.play();
    audioName.pause()
  getBox(e);
}
function getBox(e) {
    showPlay.classList.add("visible")
    //if(list.contains('active')) {
    //    e.target.classList.add("red");
   // }
   console.log(lists.classList.contains('active'));
   list.forEach((item) => {
    console.log(item)
if(!item.classList.contains('active')&&!e.target.classList.contains('active')) {
    e.target.classList.add("red");
}
console.log(item.classList.contains('active'))
})
   // if (!e.target.classList.contains('active')&&(!active))  e.target.classList.add("red");
    if (e.target.innerHTML != birdsData[1][cards[ind]].name) {
    let audio = new Audio;
    audio.src = '../audio/0a8e6de1d6ff30d.mp3'
    audio.play();
    }
   
 //console.log(e.target.innerHTML);
  let ent = e.target.innerHTML;
  birdsData[1].forEach((a) => {
    for (let i=0; i<birdsData[1].length; i++) {
      // console.log(ent);
        //console.log(a.name);
        //console.log(birdsData[1][cards[i]].name);
        ...
        if (ent === a.name) {
         //   console.log(birdsData[1][cards[i]].name);
          ...
          // console.log(ent); 
            ...
            showImg.src = a.image;
            showAudio.src = a.audio;
            showText.innerHTML = a.description;
        }
    }
   
    showName.innerHTML = ent;
